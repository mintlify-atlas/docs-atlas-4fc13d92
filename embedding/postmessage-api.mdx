---
title: "postMessage API for Embedded Emulator Control"
sidebarTitle: "postMessage API"
description: "Send commands to an embedded Infinite Mac emulator and receive status events using the browser's window.postMessage API."
---
...
- **Absolute coordinates** (`x`, `y`) — supported by Mini vMac, Basilisk II, and SheepShaver.
...
Send a mouse button press or release. `button` follows the same numbering as [`MouseEvent.button`](https://developer.mozilla.org/en-US/docs/Web/API/MouseEvent/button): `0` = left, `2` = right.
...
Supported emulators: **Mini vMac, Basilisk II, SheepShaver, Previous**.
