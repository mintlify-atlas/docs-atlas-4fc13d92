...
description: "All postMessage event types you can send to an embedded Infinite Mac emulator: pause, mouse, keyboard, and disk loading commands."
---
...
<ParamField query="x" type="number">
  ...
  mouse coordinates: **Mini vMac**, **Basilisk II**, **SheepShaver**, and
  **Snow**.
</ParamField>
...
<ParamField query="deltaX" type="number">
  Relative X delta in pixels. Used by emulators that only support relative mouse
  input: **Previous**, **DingusPPC**, and **PearPC**.
</ParamField>
...
<Note>
  ...
  II, SheepShaver, Snow) the `x`/`y` values are used; for relative-coordinate
